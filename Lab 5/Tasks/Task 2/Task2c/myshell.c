...
//Shell commands
bool suspend_proc(cmdLine* pCmdLine);
bool kill_proc(cmdLine* pCmdLine);
bool wake_proc(cmdLine* pCmdLine);
...
struct command_func commands[] = {{ "quit", &quit }, { "cd", &change_dir }, { "procs", &print_processes },
                                 { "suspend", &suspend_proc }, { "kill", &kill_proc }, { "wake", &wake_proc }};
...
bool suspend_proc(cmdLine* pCmdLine)
{
   ...
   if((*pCmdLine).argCount != 2)
      fprintf(stderr, "Wrong suspend command usage. (command: suspend <process id>)\n");
   else
   {
      sscanf((*pCmdLine).arguments[1], "%d", &to_suspend);
      kill(to_suspend, SIGTSTP);
      updateProcessStatus(processes, to_suspend, SUSPENDED);
   }
   return false;
}
...
bool kill_proc(cmdLine* pCmdLine)
{
   pid_t to_kill;
   if((*pCmdLine).argCount != 2)
      fprintf(stderr, "Wrong kill command usage. (command: kill <process id>)\n");
   else
   {
      sscanf((*pCmdLine).arguments[1], "%d", &to_kill);
      kill(to_kill, SIGINT);
      updateProcessStatus(processes, to_kill, TERMINATED);
   }
   return false;
}
...
bool wake_proc(cmdLine* pCmdLine)
{
   pid_t to_wake;
   if((*pCmdLine).argCount != 2)
      fprintf(stderr, "Wrong wake command usage. (command: wake <process id>)\n");
   else
   {
      sscanf((*pCmdLine).arguments[1], "%d", &to_wake);
      kill(to_wake, SIGCONT);
      updateProcessStatus(processes, to_wake, RUNNING);
   }
   return false;
}
