...
char* shellFeaturesStr[]={"cd","procs","suspend","kill","wake"};
int (*shellFeaturesFunc[])(cmdLine*)={&cd,&procs,&suspend,&my_kill,&wake};
...
int suspend(cmdLine* pcmdLine){
    pid_t pidToSuspend = atoi(pcmdLine->arguments[1]);
    kill(pidToSuspend,SIGTSTP);
    updateProcessStatus(processesList,pidToSuspend,SUSPENDED);
    return 1;
}

int my_kill(cmdLine* pcmdLine){
    pid_t pidToSuspend = atoi(pcmdLine->arguments[1]);
    kill(pidToSuspend,SIGINT);
    updateProcessStatus(processesList,pidToSuspend,TERMINATED);
    return 1;
}

int wake(cmdLine* pcmdLine){
    pid_t pidToSuspend = atoi(pcmdLine->arguments[1]);
    kill(pidToSuspend,SIGCONT);
    updateProcessStatus(processesList,pidToSuspend,RUNNING);
    return 1;
}
