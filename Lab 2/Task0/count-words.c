...
char *words(int count) {
  char *words;
  if(count==1) 
    //words[strlen(words)-1] = '\0'; //NOTE: the problem is that char* is saved in text segment when initialized as string. we need to change the type to char[] to modify characters. String are immutable!
	  words = "word";
	else words = "words";
  return words;
}
